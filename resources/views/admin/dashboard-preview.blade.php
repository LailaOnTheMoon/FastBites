...
@section('title', 'FastBites Admin Dashboard Preview')

@push('styles')
    <link rel="stylesheet" href="{{ asset('css/admin-dashboard-preview.css') }}">
@endpush

@section('content')
    @php
        $stats = [
            ['label' => 'Total Sales', 'value' => '$25,400', 'accent' => 'green', 'icon' => 'sales', 'trend' => '+8.4% vs last week'],
            ['label' => 'Total Orders', 'value' => '2,230', 'accent' => 'orange', 'icon' => 'orders', 'trend' => '+5.1% from yesterday'],
            ['label' => 'Active Users', 'value' => '3,458', 'accent' => 'pink', 'icon' => 'users', 'trend' => '+124 users today'],
            ['label' => 'Total Users', 'value' => '1,524', 'accent' => 'amber', 'icon' => 'group', 'trend' => '+3.2% this month'],
            ['label' => 'Total Stores', 'value' => '402', 'accent' => 'gold', 'icon' => 'store', 'trend' => '12 new this quarter'],
            ['label' => 'Active Riders', 'value' => '86', 'accent' => 'blue', 'icon' => 'rider', 'trend' => '6 currently nearby'],
        ];
    ...
    <section class="stats-grid preview-stats-grid">
        @foreach ($stats as $stat)
            <article class="stat-card preview-stat-card">
    ...
    <section class="dashboard-grid">
        <article class="panel timeline-panel preview-panel">
            ...
            <div class="chart-wrap large-chart">
                <canvas id="previewOrderTimelineChart"></canvas>
            </div>
        </article>

        <article class="panel revenue-panel preview-panel">
            <div class="panel-header">
                <div>
                    <h3>Sales Revenue</h3>
        ...
        <article class="panel donut-panel preview-panel">
            <div class="panel-header">
                <div>
                    <h3>User Status</h3>
            ...
            <div class="chart-wrap donut-chart-wrap">
                <canvas id="previewUserStatusChart"></canvas>
            </div>
    ...
    <section class="panel orders-panel preview-panel">
        <div class="panel-header panel-header-stack">
            <div>
                <h3>Recently Placed Orders</h3>
                <p>See recently placed orders across the system.</p>
            </div>
            <a href="#" class="table-filter-link">View All</a>
        </div>

        <div class="table-wrap">
            <table class="preview-orders-table">
                <thead>
                    <tr>
                        <th>Customer Name</th>
                        <th>Order ID</th>
...
@push('scripts')
    <script>
        const previewTimelineCanvas = document.getElementById('previewOrderTimelineChart');

        if (previewTimelineCanvas) {
            const timelineContext = previewTimelineCanvas.getContext('2d');
            ...
            new Chart(previewTimelineCanvas, {
                type: 'line',
                data: {
        ...
        const previewUserStatusCanvas = document.getElementById('previewUserStatusChart');

        if (previewUserStatusCanvas) {
            new Chart(previewUserStatusCanvas, {
                type: 'doughnut',
                data: {
